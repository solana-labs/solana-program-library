...
async fn setup() -> (ProgramTestContext, StakePoolAccounts, Fee) {
    ...
    stake_pool_accounts
        ...
        let validator_fee = Fee {
            numerator: 1,
            denominator: 100
        };

    (context, stake_pool_accounts, validator_fee)
}

#[tokio::test]
async fn success() {
    let (mut context, stake_pool_accounts, _) = setup().await;
    ...
    let validator_fee = Fee {
        numerator: stake_pool.validator_fee.numerator + 1,
        denominator: stake_pool.validator_fee.denominator + 1,
    };
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[instruction::set_fee(
            ...
            FeeType::Validator(validator_fee),
        )],
    ...
    assert_eq!(stake_pool.validator_fee, validator_fee);
}
