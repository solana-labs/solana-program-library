...
#[tokio::test]
async fn test_not_manager_try_to_remove_validator_from_pool() {
    let (
        mut banks_client,
        payer,
        recent_blockhash,
    ...
    match transaction_error {
        ...
        _ => {
            panic!("Wrong error occurs while not an manager try to remove validator stake address")
        }
    }
}

#[tokio::test]
async fn test_not_manager_try_to_remove_validator_from_pool_without_signature() {
    let (
        mut banks_client,
        payer,
        recent_blockhash,
