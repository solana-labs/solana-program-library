        ...
        it('should successfully decode StakePoolAccount account data', () => {
            ...
            const expectedData = new schema.StakePoolAccount({
                ...
                'manager': new schema.PublicKey({
                    'value': new BN('dc23cda2ad09ddec126f89ed7f67d06a4d167cca996503f1a1b3b5a13625964f', 'hex')
                }),
                ...
                'depositAuthority': new schema.PublicKey({
                    'value': new BN(new Buffer('5911e7451a1a854fdc9e495081790f293eba623f8ec7e2b9d34a5fd25c7009bb', 'hex'))
                }),
                'withdrawBumpSeed': 255,
                'validatorList': new schema.PublicKey({
                    'value': new BN(
                        '7103ba4895b8804263197364da9e791db96ec8f0c8ca184dd666e69013838610', 'hex'
                    )
                }),
                'reserveStake': new schema.PublicKey(
                    {
                        'value': new BN(
                            '74a5b1ab8442103baa8bd39ab8494eb034e96035ac664e1693bb3eef458761ee', 'hex'
                        )
                    }
                ),
                'poolMint': new schema.PublicKey(
                    {
                        'value': new BN(
                            '8722bf107b95d2620008d256b18c13fa3a46ab7f643c24cf7656f57267563e00', 'hex'
                        )
                    }
                ),
                'managerFeeAccount': new schema.PublicKey({
                    'value': new BN(
                        new Buffer('b783b4dcd341cbca22e781bbd49b2d16908a844a21b98e26b69d44fc50e1db0f', 'hex')
                    )
                }),
                'tokenProgramId': new schema.PublicKey({
                    'value': new BN(
                        'a900ff7e85f58c3a91375b5fed85b41cac79ebce46e1cbd993a165d7e1f6dd06', 'hex'
                    )
                }),
                ...
                if (expectedData[key] instanceof BN) {
                    ...
                    for (const subkey in decodedData[key]) {
                        if (decodedData[key][subkey] instanceof BN) {
                            ...
                            assert.deepStrictEqual(expectedData[key][subkey]
