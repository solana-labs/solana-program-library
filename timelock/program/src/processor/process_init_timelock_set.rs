...
pub fn process_init_timelock_set(
    ...
    if !rent.is_exempt(
        timelock_set_account_info.lamports(),
        timelock_set_account_info.data_len(),
    ) {
        return Err(TimelockError::NotRentExempt.into());
    }
    if !rent.is_exempt(
        governance_holding_account_info.lamports(),
        governance_holding_account_info.data_len(),
    ) {
        return Err(TimelockError::NotRentExempt.into());
    }
    if !rent.is_exempt(
        admin_mint_account_info.lamports(),
        admin_mint_account_info.data_len(),
    ) {
        return Err(TimelockError::NotRentExempt.into());
    }
    if !rent.is_exempt(
        voting_mint_account_info.lamports(),
        voting_mint_account_info.data_len(),
    ) {
        return Err(TimelockError::NotRentExempt.into());
    }
    if !rent.is_exempt(
        yes_voting_mint_account_info.lamports(),
        yes_voting_mint_account_info.data_len(),
    ) {
        return Err(TimelockError::NotRentExempt.into());
    }
    if !rent.is_exempt(
        no_voting_mint_account_info.lamports(),
        no_voting_mint_account_info.data_len(),
    ) {
        return Err(TimelockError::NotRentExempt.into());
    }
    if !rent.is_exempt(
        signatory_mint_account_info.lamports(),
        signatory_mint_account_info.data_len(),
    ) {
        return Err(TimelockError::NotRentExempt.into());
    }
    if !rent.is_exempt(
        admin_validation_account_info.lamports(),
        admin_validation_account_info.data_len(),
    ) {
        return Err(TimelockError::NotRentExempt.into());
    }
    if !rent.is_exempt(
        signatory_validation_account_info.lamports(),
        signatory_validation_account_info.data_len(),
    ) {
        return Err(TimelockError::NotRentExempt.into());
    }
    if !rent.is_exempt(
        voting_validation_account_info.lamports(),
        voting_validation_account_info.data_len(),
    ) {
        return Err(TimelockError::NotRentExempt.into());
    }
