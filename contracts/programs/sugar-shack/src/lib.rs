...
pub mod sugar_shack {
    ...
    pub fn update_marketplace_properties<'info>(
        ...
        assert_eq!(ctx.accounts.authority.key(), ctx.accounts.marketplace_props.authority);
        match authority {
    ...
    pub fn purchase<'info>(
        ...
        let amount_to_pay_marketplace = basis_point_of_price.safe_mul(ctx.accounts.marketplace_props.share as u64)?;
        invoke(
        ...
        let basis_point_of_remaining_price_allocation = total_remaining_price_allocation.safe_div(10000 as u64)?;
        ...
        for share in creator_shares.into_iter() {
            ...
            let amount_to_pay_creator = basis_point_of_remaining_price_allocation.safe_mul((share as u64).safe_mul(100 as u64)?)?;
            invoke(
                &system_instruction::transfer(
                    &ctx.accounts.purchaser.key(),
            ...
            amount_paid_out_to_creators = amount_paid_out_to_creators.safe_add(amount_to_pay_creator)?;
            creator_data.push([current_creator_info.key().as_ref(), &[share]].concat());
        }
        total_remaining_price_allocation = total_remaining_price_allocation.safe_sub(amount_paid_out_to_creators)?;
        ...
        let seeds: &[&[u8]] = &[price_seed.as_ref(), &[*ctx.bumps.get("listing_delegate").unwrap()]];
    ...
    pub fn withdraw_fees<'info>(
        ...
        // This instruction must be signed by the authority to the marketplace
        assert_eq!(ctx.accounts.authority.key(), ctx.accounts.marketplace_props.authority);
        let marketplace_props_balance_after_withdrawal = ctx.accounts.marketplace_props.to_account_info().lamports().safe_sub(lamports_to_withdraw)?;
        
        // The marketplace props account must be left with enough funds to be rent exempt after the withdrawal
        assert!(Rent::get()?.is_exempt(marketplace_props_balance_after_withdrawal, MARKETPLACE_PROPERTIES_SIZE));

        // Transfer lamports from props PDA to fee_payout_recipient
        **ctx.accounts.marketplace_props.to_account_info().try_borrow_mut_lamports()? = marketplace_props_balance_after_withdrawal;
        **ctx.accounts.fee_payout_recipient.try_borrow_mut_lamports()? = ctx.accounts.fee_payout_recipient.lamports().safe_add(lamports_to_withdraw)?;
        Ok(())
    }
}