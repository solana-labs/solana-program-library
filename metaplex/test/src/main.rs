use {
    ...
    spl_token_vault::{
        ...
        state::{
            ExternalPriceAccount, Metaplex, MetaplexState, SafetyDepositBox,
            MAX_EXTERNAL_ACCOUNT_SIZE, MAX_VAULT_SIZE, PREFIX,
        },
    },
    std::str::FromStr,
};
...
fn activate_vault(app_matches: &ArgMatches, payer: Keypair, client: RpcClient) -> Option<Pubkey> {
    ...
    let vault: Metaplex = try_from_slice_unchecked(&vault_account.data).unwrap();
    ...
    let updated_vault: Metaplex = try_from_slice_unchecked(&updated_vault_data.data).unwrap();
    if updated_vault.state == MetaplexState::Active {
...
fn combine_vault(app_matches: &ArgMatches, payer: Keypair, client: RpcClient) -> Option<Pubkey> {
    ...
    let updated_vault_data = client.get_account(&vault_key).unwrap();
    let updated_vault: Metaplex = try_from_slice_unchecked(&updated_vault_data.data).unwrap();
    if updated_vault.state == MetaplexState::Combined {
