...
impl Pack for SwapInfo {
    ...
    fn unpack_from_slice(input: &[u8]) -> Result<Self, ProgramError> {
        let input = array_ref![input, 0, 259];
        #[allow(clippy::ptr_offset_with_cast)]
        let (is_initialized, nonce, token_program_id, token_a, token_b, pool_mint, token_a_mint, token_b_mint, swap_curve) =
            array_refs![input, 1, 1, 32, 32, 32, 32, 32, 32, 65];
        Ok(Self {
    ...
    fn pack_into_slice(&self, output: &mut [u8]) {
        let output = array_mut_ref![output, 0, 259];
        let (is_initialized, nonce, token_program_id, token_a, token_b, pool_mint, token_a_mint, token_b_mint, swap_curve) =
            mut_array_refs![output, 1, 1, 32, 32, 32, 32, 32, 32, 65];
