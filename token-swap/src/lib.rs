...
impl std::convert::From<solana_sdk::pubkey::PubkeyError> for solana_sdk::program_error::ProgramError {
    fn from(e: solana_sdk::pubkey::PubkeyError) -> Self {
        unimplemented!();
    }
}
...
impl<'a> State {
     ...
     pub fn create_token_account(
         ...
         let instruction = token::instruction::Instruction::NewAccount;
         instruction.serialize(&mut instruction_data)?;
 
         let account_addr = Pubkey::create_program_address(&[Self::to_str(instance_id), kind], program_id)?;
         let authority = Pubkey::create_program_address(&[Self::to_str(instance_id), "authority"], program_id)?;
