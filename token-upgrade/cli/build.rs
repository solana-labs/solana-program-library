...
fn build_sbf(program_directory: &Path) {
...
fn main() {
    ...
    if is_debug && build_dependent_programs {
        ...
        let spl_token_upgrade_dir = cwd
            .parent()
            .expect("Unable to get parent directory of current working dir")
            .join("program");
        rerun_if_changed(&spl_token_upgrade_dir);
        ...
        build_sbf(&spl_token_upgrade_dir);
        build_sbf(&spl_token_dir);
        build_sbf(&spl_token_2022_dir);
        build_sbf(&spl_associated_token_account_dir);
    }
