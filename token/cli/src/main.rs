...
pub static FALLBACK_PROGRAM_ID: Pubkey = spl_token::ID;
...
async fn command_create_token(
    ...
    let program_id = config.program_id.unwrap_or(FALLBACK_PROGRAM_ID);
...
async fn command_authorize(
    ...
    let (previous_authority, program_id) = if !config.sign_only {
        ...
        (
            COption::None,
            config.program_id.unwrap_or(FALLBACK_PROGRAM_ID),
        )
    };
    println_display(
        config,
...
async fn command_unwrap(
    ...
    let address = address.unwrap_or_else(|| {
        get_associated_token_address_with_program_id(&wallet_address, &native_mint, &program_id)
    });
...
async fn command_revoke(
    ...
    let (delegate, program_id) = if !config.sign_only {
        ...
        (delegate, config.program_id.unwrap_or(FALLBACK_PROGRAM_ID))
    };
...
async fn command_sync_native(
    ...
    let program_id = config.check_account(&native_account_address, None).await?;
    let tx_return = handle_tx(
        &CliSignerInfo {
            signers: bulk_signers,
        },
...
async fn main() -> Result<(), Error> {
    ...
    let config = {
        ...
        let program_id = pubkey_of(matches, "program_id");
...
mod tests {
    ...
    async fn mint_tokens(
        ...
        command_mint(
            ...
            recipient,
            MintInfo {
                program_id: config.program_id.unwrap(),
                address: mint,
                decimals: 0,
            },
            payer.pubkey(),
